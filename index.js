...
app.use(session({
    secret: '33748627154566784e46033718acdb8eb87c0e4438e768787c1be48908446fbe37ed346373c2eca4f044f0b917bc397bf12ccef7ce1884b5d3cb93492425c4e1',
    resave: false,
...
app.get('/averageScores', authenticate, async (req, res) => {
    try {
        const user = await User.findOne({ name: new RegExp(`^${req.session.username}$`, 'i') });
...
app.post('/submitQuiz', authenticate, async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: 'Quiz submitted successfully', averageScore, allScores: user.averageScores });
    } catch (error) {
...
app.post("/AI_Chat", async (req, res) => {
    try {
        ...
        if (err.message?.includes("429") || err.status === 429) {
            return res.status(429).json({ error: "AI Quota exceeded. Please try again later or check your billing." });
        }
